						...
						<tr>
							<td class="d-flex align-items-center gap-2">
								<?php if (!empty($product['image'])): ?>
									<img src="<?= BASE_URL ?>/public/<?= htmlspecialchars($product['image']) ?>" alt="" style="width: 40px; height: 40px; object-fit: cover;" class="rounded">
								<?php else: ?>
							...
							<td class="text-end">
								<a class="btn btn-sm btn-outline-secondary" href="<?= BASE_URL ?>/?page=admin.product.add&id=<?= (int)$product['id'] ?>">Edit</a>
								<a class="btn btn-sm btn-outline-warning" href="<?= BASE_URL ?>/?page=admin.product.toggle&id=<?= (int)$product['id'] ?>&p=<?= (int)$page ?>">
									<?= $isAvailable ? 'Disable' : 'Enable' ?>
								</a>
								<form method="post" action="<?= BASE_URL ?>/?page=admin.product.delete" class="d-inline" onsubmit="return confirm('Delete this product?');">
			...
			<nav class="mt-3">
				<ul class="pagination justify-content-center mb-0">
					<li class="page-item <?= ($page ?? 1) <= 1 ? 'disabled' : '' ?>">
						<a class="page-link" href="<?= BASE_URL ?>/?page=admin.products&p=<?= max(1, ($page ?? 1) - 1) ?>">&lt;</a>
					</li>
					<li class="page-item active"><span class="page-link"><?= (int)($page ?? 1) ?></span></li>
					<li class="page-item <?= ($page ?? 1) >= ($pages ?? 1) ? 'disabled' : '' ?>">
						<a class="page-link" href="<?= BASE_URL ?>/?page=admin.products&p=<?= min(($pages ?? 1), ($page ?? 1) + 1) ?>">&gt;</a>
					</li>
				</ul>
			</nav>
		<?php endif; ?>
