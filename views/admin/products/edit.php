	...
	<form action="<?= BASE_URL ?>/?page=admin.product.save" method="post" enctype="multipart/form-data" class="card p-4 shadow-sm">
		...
		<div class="row g-3">
			...
			<div class="col-md-3">
				...
				<input type="number" name="price" class="form-control" min="0.1" step="0.1" value="<?= htmlspecialchars($product['price'] ?? '') ?>" required>
			</div>
			<div class="col-md-3">
				<label class="form-label">Availability</label>
				<?php $isAvailable = (int)($product['is_available'] ?? $product['available'] ?? 1); ?>
				<select name="is_available" class="form-select">
					<option value="1" <?= $isAvailable ? 'selected' : '' ?>>Available</option>
					<option value="0" <?= !$isAvailable ? 'selected' : '' ?>>Unavailable</option>
				</select>
			</div>
			<div class="col-md-6">
